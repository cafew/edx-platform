...
class TestMixedModuleStore(CommonMixedModuleStoreSetup):
    ...
    def test_unit_stuck_in_published_mode_after_delete(self, default_ms):
        ...
        component_one = self.store.create_child(
            self.user_id,
            vertical.location,
            'html',
            block_id='html_one'
        )
        component_two = self.store.create_child(
            self.user_id,
            vertical.location,
            'html',
            block_id='html_two'
        ...
        # Change a vertical component, then check that there now are changes
        component_two = self.store.get_item(component_two.location)
        component_two.display_name = 'Changed Display Name'
        self.store.update_item(component_two, self.user_id)
        ...
        # # Now delete the component and verify that the unit has changes
        self.store.delete_item(component_two.location, self.user_id)
